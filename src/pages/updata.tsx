...
import { updataFrom, updataSchema } from "../models"
...
const Updatable = () => {
    const { register, handleSubmit, formState: { errors } } = useForm<updataFrom>({
        resolver: yupResolver(updataSchema),
        defaultValues: async () => {
            if (id) {
                return await fetchUpdata(id)
            }
        }
    })
    ...
    const onSubmit = async (data: updataFrom) => {
        try {
            if (id) {
    ...
    const fetchUpdata = async (id: string) => {
    ...
    return <>
        ...
        <div className="p-4 sm:ml-64">
            <div className="relative overflow-x-auto  sm:rounded-lg m-5  w-50    ">
                <h4 className="mt-10 mb-10">Cập Nhập sản phẩm</h4>
        ...
        bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Cập nhập</button>
                </form>
